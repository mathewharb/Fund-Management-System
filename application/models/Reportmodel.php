...
WHERE accounts_name=a.accounts_name AND user_id='".$this->session->userdata('user_id')."' ORDER BY trans_id DESC LIMIT 1) 
as balance FROM accounts  as a WHERE user_id='".$this->session->userdata('user_id')."'");
$result=$query->result();
return $result;	
}
...
return $this->db->query("SELECT trans_date,note,dr,cr,bal FROM
 transaction WHERE accounts_name='".$account."' AND user_id='".$this->session->userdata('user_id')."' AND trans_date between
 '".$from_date."' AND '".$to_date."'")->result();	
}else{
return $this->db->query("SELECT trans_date,note,dr,cr,bal FROM 
transaction WHERE accounts_name='".$account."' AND user_id='".$this->session->userdata('user_id')."' AND $trans_type>0 AND trans_date
